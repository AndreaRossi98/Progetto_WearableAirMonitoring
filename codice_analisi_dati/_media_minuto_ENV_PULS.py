...
filename_EM = 'env.txt'       #Inserire nome del file di Environmental Monitor
filename_Pulse = 'pulse.txt'    #Inserire nome del file di Pulse oximeter
...
dataE['PM1.0'] = dataE['PM1.0'].apply(float)
...
for i in range(1, dataE['tempo_trascorso'].get(length-1)):
    if dataE['temperature'].get(j) == None:
        ...
        if dataE['PM1.0'].get(j) == 0:
            PM1_null = PM1_null +1    
        else:
            PM1 = PM1 + dataE['PM10'].get(j)
...
PM10s = pd.DataFrame(mean_PM1, columns=["PM10"])


risultatoE = pd.concat([Minutes,VOCs,CO2s,COs,NO2s,PM1s,PM2p5s,PM10s], axis=1)
...
#SALVATAGGIO DATI SU TXT
np.savetxt('media_Pulse.txt', risultatoP, fmt='%.1f')
