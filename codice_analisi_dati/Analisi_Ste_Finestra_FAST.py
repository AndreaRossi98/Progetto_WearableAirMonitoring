...
filename = 'S1_IMU0.txt'
#A:sit.wo.su, B:sit, C:supine, D:prone, E:lyingL, F:lyingR, G:standing, I:stairs, L:walkS, M:walkF, N:run, O:cyclette
window_size = 200  # samples inside the window (Must be >=SgolayWindowPCA). 1 minute = 600 samples
...
incr = 100  # Overlapping between a window and the following. 1=max overlap. MUST BE >= SgolayWindowPCA. The higher the faster
...
data.columns = ['index', 'DevID', 'B', 'C', 'nthvalue', '1', '2', '3', '4']
...
length = len(data)
ncycles = 2100 if stop == 0 else stop
