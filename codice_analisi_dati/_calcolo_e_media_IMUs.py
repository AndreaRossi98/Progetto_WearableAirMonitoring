...
incr = 430  #300  # Overlapping between a window and the following. 1=max overlap. MUST BE >= SgolayWindowPCA. The higher the faster
...
def plotupdate():
    ...
    # CREAZIONE FINESTRA 3: FREQUENZA RESPIRATORIA
        try:
            ...
            plt.ylabel('Resp/min (bpm)')
            plt.title('Thorax frequency')
            plt.subplot(2, 1, 2)
            plt.plot(indexes, [PCA_A[x][0] for x in range(len(indexes))], color='blue', marker='o')
            plt.ylabel('Resp/min (bpm)')
            plt.title('Abdomen frequency')
            plt.figure(6)
            #plt.subplot(3, 1, 3)
...
'''
risultato_parz = pd.DataFrame(Freq_media, columns=["Frequenza_media"])
#tot.to_csv('total_params_out.csv')
#np.savetxt('PazienteSoloFreq.txt', fB_Tot, fmt='%f') #breath by breath
np.savetxt('FREQ_IMU.txt', risultato_parz, fmt='%.2f')    #meanFreq     frequenza media ogni 30 sec
#np.savetxt('PazienteFreq.txt', dftot, fmt='%f')        
"""
if timeinminutes:
