    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.addbuttonlist:
                //that's the + button, which redirects to the patient dialog
                dialogAddPatient();
            break;
            ...
            case R.id.addbuttondialog:  //that's the ADD button in the dialog to add a patient
                addPatientToTheList();
            break;
...
//                else{
    ...
    private void loadPatientsList(){
        ...
        if (items_cardViews == null || items_cardViews.size()==0) {  // se la lista è vuota
            items_cardViews=new ArrayList<>();  //crea array vuoto
            dialogAddPatient();
