...
dummy_message_IMU = 0
dummy_message_IMU1 = 0
dummy_message_IMU2 = 0
dummy_message_IMU3 = 0
...
E_fine_ora = 0
E_fine_secondo = 0
E_intervallo_tempo = 0

P_inizio_millisecondo = 0
P_inizio_secondo = 0
P_inizio_minuto = 0
P_inizio_ora = 0
P_fine_millisecondo = 0
P_fine_secondo = 0
P_fine_minuto = 0
P_fine_ora = 0
P_intervallo_tempo = 0

I_inizio_millisecondo = 0
I_inizio_secondo = 0
I_inizio_minuto = 0
I_inizio_ora = 0
I_fine_millisecondo = 0
I_fine_secondo = 0
I_fine_minuto = 0
I_fine_ora = 0
I_intervallo_tempo = 0
    ...
    if (data[0].get(i) == "6" and data[1].get(i)!= "0"):     #SELEZIONA DATI DA ENVIRONMENTAL MONITOR ed esclude i pacchetti iniziali e finali senza valori
        ...
        if(data[0].get(i) == IMU1 or data[0].get(i) == IMU2 or data[0].get(i) == IMU3):
            ...
            if(data[2].get(i) == D):
                ...
                if(data[0].get(i) == IMU1):
                    dummy_message_IMU1 = dummy_message_IMU1 +1
                if(data[0].get(i) == IMU2):
                    dummy_message_IMU2 = dummy_message_IMU2 +1
                if(data[0].get(i) == IMU3):
                    dummy_message_IMU3 = dummy_message_IMU3 +1
...
np.savetxt(step+'_ENV_'+'.txt', risultatoE, fmt='%s')

#PULSOSSIMETRO SALVATAGGIO TXT
P_Device = pd.DataFrame(P_dev)
...
np.savetxt(step+'_PULSE'+'.txt', risultatoP, fmt='%s')
...
I_VIRGOLA = pd.DataFrame(I_virgola)
...
risultatoI = pd.concat([I_DEV,I_VIRGOLA,I_UNO,I_VIRGOLA,I_DUE,I_VIRGOLA,I_TRE,I_VIRGOLA,I_QUATTRO,I_VIRGOLA,I_CINQUE,I_VIRGOLA,I_SEI,I_VIRGOLA,I_SETTE,I_VIRGOLA,I_GIORNO], axis = 1)  #I_MESE,I_ORA,I_MINUTO,I_SECONDO,I_MILLISECONDO
np.savetxt(step+'_IMU'+'.txt', risultatoI, fmt='%s')
...
if(count_environmental != 0):
    ...
    print("Environmental dura", E_intervallo_tempo,"secondi, circa ",E_intervallo_tempo/25, "campioni (stima uno ogni 25 secondi)")
...
print("Perso IMU1 ", dummy_message_IMU1)
print("Perso IMU2 ", dummy_message_IMU2)
print("Perso IMU3 ", dummy_message_IMU3)

f = open(step+"_INFO.txt", "x")
...
+ " Information loss Environmental: " + str(informationloss_E)
...
+ "Perso IMU1 "+str(dummy_message_IMU1)
+ "\nPerso IMU2 "+str(dummy_message_IMU2)
+ "\nPerso IMU3 "+str(dummy_message_IMU3))
